...
const pageItems = 10; // Stored max number of items to show on each page


//showPage function
const showPage = (list, page) => { //list, page parameters to be passed in
  let startIndex = ( page * pageItems ) - pageItems; //first item
  let endIndex = page * pageItems; //last item
  for (let i = 0; i < list.length; i++) {
    //currently unexecuted code * currently showing all 'li'
    if (i >= startIndex && i <= endIndex) {
      list[i].style.display = 'block'; //show list item index
    } else {
...
const appendPageLinks = (list) => {
  ...
  for (let i = 0; i < pageItems; i++ ) { //for every page, add li and a tags with the page number text
    a.className = 'active'; //'a' link active
    a.href = '#'; //sets link address
    a.textContent = 1; //'a' links to start at page 1
